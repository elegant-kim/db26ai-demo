...
## 5. 공존·전환·롤백 (D3) — ✅ 종료 2026-09-05 (Phase 6-1: 레거시 삭제, SPA 단일 서빙. 아래는 이식 기간의 기록)
...
| # | 리스크 | 대응 |
|---|---|---|
| R1 | 채팅형 화면(nl2sql·vector)이 사이드바 없이 **너무 넓어져** 말풍선 가독성이 떨어질 수 있다 | ~~스레드 폭 960 중앙 정렬~~ → 사용자 판단(2026-09-05): **전체 폭이 낫다** — 서브탭 간 폭 불일치가 더 거슬린다. 말풍선 80% 제한으로 가독성은 유지 |
...
4. 채팅 스레드 폭(R1) — 5-5 에서 → ~~960 중앙 정렬~~ **✅ 재확정: 전체 폭(1400 컨테이너)** — 사용자 지시 2026-09-05 "첫 탭만 좁아 보인다, 모든 화면을 같은 폭으로". 말풍선만 80% 로 제한한다
