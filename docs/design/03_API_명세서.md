...
| Method | 경로 | 요청 | 설명 | 구현 |
|---|---|---|---|---|
| `POST` | `/api/awr/analyze` | multipart 파일 | AWR HTML 파일 업로드 → 파싱 (23개 섹션) → LLM 분석 (8개 섹션 보고서) | `app/routers/awr.py:35` |
| `POST` | `/api/awr/followup` | AWRFollowupRequest | AWR 분석 결과에 대한 후속 질문 | `app/routers/awr.py:121` |
| `GET` | `/api/awr/source/{session_id}` | — | AWR HTML 원문 보기 | `app/routers/awr.py:156` |
