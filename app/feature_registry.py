...
_F = [
 ...
 ("vector", "환경 — 이 DB 에 무엇이 준비돼 있나", "VECTOR 타입 테이블 · DB 안 ONNX 모델 · HNSW/Hybrid Vector Index 를 시연 시작 전에 한 화면에서",
  "시연의 시작점(기본 진입). 소개 카드 → 현재 설정 → Hybrid Vector Index 상태. 바꾸는 것은 「내부」에서.",
 ...
 ("vector", "Hybrid Vector Index 생성", "Vector Store 탭에서 인덱스 상태 확인 · 생성(옛 Oracle Text 인덱스 대체)",
  "청크 수 × 0.2초. 업로드마다 CTX_DDL.SYNC_INDEX 로 새 청크가 반영된다. 실행되는 DDL 이 화면에 남는다.",
  "/vector?sub=internals", "hybrid vector index create 생성 ctx_ddl sync"),
