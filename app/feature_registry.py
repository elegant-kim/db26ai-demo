...
_F = [
 ...
 ("nl2sql", "환경 확인 3종", "profile(프로필 속성) · acl(네트워크 ACL) · credential(크리덴셜) 을 버튼으로 조회",
  "Select AI 가 안 될 때 어디가 문제인지 짚는 자리다. ACL 이 없으면 ORA-24247, 키가 틀리면 ORA-20404 로 실패한다. 조회 SQL 이 함께 표시된다.",
  "/nl2sql?sub=ask", "env acl credential 크리덴셜 네트워크 권한 ORA-24247 ORA-20404 진단"),
