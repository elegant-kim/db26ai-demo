...
_F = [
 ...
 ("nl2sql", "AI 프로필 선택", "Select AI 프로필(LLM 제공자·모델·참조 테이블 묶음) 전환",
  ...
  "nl2sql:AI 프로필", "profile 프로필 groq gemini select ai"),
 ("nl2sql", "실행 모드 7종", "runsql·showsql·narrate·explainsql·showprompt·summarize·chat",
  ...
  "nl2sql:실행 모드", "action runsql showsql narrate explainsql showprompt summarize chat 모드"),
 ("nl2sql", "예시 질문", "SH 스키마용 데모 질문 14종",
  "무엇을 물어야 할지 막힐 때. 난이도 순으로 배치돼 있다.",
  "nl2sql:예시 질문", "sample 예시 질문 데모"),
 ...
 ("graph", "그래프 생성/삭제", "SH 테이블(CUSTOMERS·PRODUCTS·SALES) 위에 SQL Property Graph 정의",
  ...
  "graph:그래프 관리", "graph 그래프 생성 property sql/pgq"),
 ("graph", "SQL vs SQL/PGQ 비교", "같은 질문을 기존 JOIN 과 그래프 질의로 각각 실행",
  "이 탭의 핵심. 3가지 질문(구매 목록 / 제품별 매출 Top-10 / 추천 시스템 기초)이 **양쪽 완전 동일한 결과**를 낸다.",
  "graph:SQL vs SQL/PGQ 비교", "compare join pgq 비교 graph_table"),
 ("graph", "관계 탐색 (패턴 매칭)", "MATCH 패턴으로 관계를 따라가는 질의 3종",
  "JOIN 으로는 쓰기 힘든 질의를 보여줄 때. 2-hop(고객→제품←고객) 이 대표적.",
  "graph:그래프 분석", "match 패턴 관계 탐색 2-hop"),
 ("graph", "그래프 시각화", "정점·간선을 화면에 그려서 관계를 눈으로 보여준다",
  "표만으로 감이 안 올 때.",
  "graph:그래프 시각화", "visualize 시각화 그래프"),
 ("graph", "실행 쿼리 확인", "V$SQL 에서 GRAPH_TABLE 관련 최근 쿼리 조회",
  "그래프 질의가 실제로 어떤 SQL 인지 보여줄 때.",
  "graph:쿼리", "v$sql 실행 쿼리 graph_table"),
