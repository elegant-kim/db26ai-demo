...
const props = withDefaults(defineProps<{ messages: ChatMessage[]; emptyText?: string; maxHeight?: string; loadingText?: string }>(),
  { emptyText: '', maxHeight: '460px', loadingText: '답변 생성 중…' })
...
<template>
  <div ref="box" class="overflow-auto flex flex-col gap-3 pr-1" :style="{ maxHeight }">
    <p v-if="!messages.length && emptyText" class="text-sm m-0 py-2" style="color: var(--text-muted);">{{ emptyText }}</p>
    <div v-for="(m, i) in messages" :key="i" class="flex" :class="m.role === 'user' ? 'justify-end' : 'justify-start'">
      <div v-if="m.role === 'user'" class="max-w-[80%] rounded-xl px-3.5 py-2 text-sm whitespace-pre-wrap" style="background: var(--accent-primary); color: var(--text-on-accent);">{{ m.content }}</div>
