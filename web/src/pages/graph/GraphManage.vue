...
<template>
  <div class="flex flex-col gap-5">
    ...
    <Card title="SQL Property Graph — Oracle 26ai 의 차별성" :icon="Boxes">
      <p class="text-sm m-0 mb-3" style="color: var(--text-secondary); line-height: 1.7;">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="rounded-md px-3.5 py-3" style="background: var(--bg-surface); border: 1px solid var(--border-default);">
          <div class="text-sm font-semibold mb-1" style="color: var(--text-primary);">기존 그래프 DB (Neo4j 등)</div>
          <div class="text-xs" style="color: var(--text-secondary); line-height: 1.6;">별도 DB 에 데이터 복제 필요. Cypher 등 전용 언어. 트랜잭션 일관성 보장 어려움.</div>
        </div>
        <div class="rounded-md px-3.5 py-3" style="background: var(--accent-primary-soft); border: 1px solid var(--accent-primary);">
          <div class="text-sm font-semibold mb-1" style="color: var(--accent-primary);">Oracle Property Graph (SQL/PGQ)</div>
          <div class="text-xs" style="color: var(--text-secondary); line-height: 1.6;"><strong>기존 테이블 그대로 사용 — 복제 없음.</strong> ISO 표준 SQL/PGQ. MVCC 트랜잭션 일관성. JSON/VECTOR 와 통합.</div>
        </div>
      </div>
    </Card>
