...
<template>
  <div class="w-full flex flex-col gap-4">
    <div v-if="v.docsLoaded && !v.docs.length" class="px-3 py-2.5 rounded-md text-sm" style="background: var(--accent-warm-soft); border-left: 3px solid var(--accent-warm); color: var(--text-primary);">아직 올린 문서가 없습니다 — 「적재」에서 PDF 를 올리면 청킹 → 임베딩 → 인덱싱이 DB 안에서 돕니다.</div>
